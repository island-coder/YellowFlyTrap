...
def classify(file):
    ...
    nesi_exclude=cv2.bitwise_not(mask_wo_artefacts)

    img_wf_mask,mask= hsv.hsv_sep_wf(img)
    mask=cv2.bitwise_and(mask,mask,mask=nesi_exclude)
    ...
    contours, hierarchy = cv2.findContours(mask_wo_artefacts, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    for cnt in contours:
        if cv2.contourArea(cnt) >2000 and cv2.contourArea(cnt) <5000 and cv2.arcLength(cnt,True)<1000:
